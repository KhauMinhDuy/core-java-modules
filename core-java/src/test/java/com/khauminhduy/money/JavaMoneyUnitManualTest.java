...
public class JavaMoneyUnitManualTest {
	...
	public void givenAmounts_whenStringified_thanEquals() {
		CurrencyUnit usd = Monetary.getCurrency("USD");
		MonetaryAmount fastAmountUSD = Monetary.getDefaultAmountFactory().setCurrency(usd).setNumber(200).create();
		Money money = Money.of(12, usd);
		FastMoney fastMoney = FastMoney.of(2, usd);

		assertEquals("USD", usd.toString());
		assertEquals("USD 200.00", fastAmountUSD.toString());
		assertEquals("USD 12.00", money.toString());
		assertEquals("USD 2.00", fastMoney.toString());
	...
	public void givenAmount_whenDivided_thanThrowsException() {
		MonetaryAmount oneDolar = Monetary.getDefaultAmountFactory().setCurrency("USD").setNumber(1).create();
		MonetaryAmount divide = oneDolar.divide(3);
//		fail(); // if no exception
		assertEquals("USD 0.33", divide.toString());
	}
